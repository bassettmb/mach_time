...
int
clock_task_getres(mach_port_t task, struct timespec *ts)
{
    ts->tv_sec = 0;
...
int
clock_task_gettime(mach_port_t task, struct timespec *ts)
{
...
int
clock_task_settime(mach_port_t task, const struct timespec *ts)
{
    errno = EINVAL;
    return -1;
}
